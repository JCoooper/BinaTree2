...
public class BinaryTree {
	...
	public void insert(String o) {
		root = subtreeInsert(root, o);
	}

	private Node subtreeInsert(Node topNode, String o) {
		if (topNode == null) {
			topNode = new Node(o);
		} else if (o.compareTo(topNode.record) <= 0) {
			topNode.left = subtreeInsert(topNode.left, o);
		} else {
			topNode.right = subtreeInsert(topNode.right, o);
		}
		return topNode;
	}
	...
	private int subtreeCountMathches(Node topNode, String key) {
		int m = 0;
		if (topNode == null) {
			return 0;
		} else {
			int x = subtreeCountMathches(topNode.left, key);
			int y = subtreeCountMathches(topNode.right, key);
	...
	private String subtreeMaxRecord(Node topNode) {
		if (topNode == null) {
			...
			String x = topNode.record;

			if (topNode.record != null) {
				x = subtreeMaxRecord(topNode.right);
			}
			return x;
		}
	}
	...
	private String subtreeMinRecord(Node topNode) {
		if (topNode == null) {
			...
			if (topNode.left != null) {
				x = subtreeMinRecord(topNode.left);
			}
			return x;
		}
	}
